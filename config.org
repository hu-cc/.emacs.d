...
(setq-default mode-line-format
      '("%e"
        " ["
         (:propertize "%b" face (:inherit font-lock-keyword-face :weight bold))
        " ] --- "
        
         (:propertize which-func-format face (:inherit font-lock-function-name-face))
...
#+BEGIN_SRC emacs-lisp
  (use-package eglot
    ...
    ((c-mode c++-mode c-ts-mode c++-ts-mode) . (lambda () (add-hook 'before-save-hook #'eglot-format-buffer nil t)))
  ...
  (use-package cape
    :ensure t
    :init
    ;; 建议将 cape 挂载到特定 hook，确保 eglot 优先
    (add-hook 'eglot-managed-mode-hook
              (lambda ()
                ;; 将 cape-file 放到补全列表后面，作为 eglot 的补充
